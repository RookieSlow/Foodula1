...
- [ ] P0：增强挡位出牌要求反馈，显示有效要求值、已出/要求数量和缺牌引擎故障预警；
  保持现有缺牌惩罚规则不变。
- [ ] P1：选中卡牌缩放并上移，保持布局槽位不变，验证多选、特技单选和遮罩边界。
- [ ] P1：为抽牌堆和弃牌堆增加牌背叠放、实际卡牌缩略图、数量徽标和牌堆变化刷新。
...
- [x] 增加 RaceRanking 回归测试，并让纯层比赛模拟覆盖玩家 DNF 后其余赛车继续比赛；
  Unity EditMode 全量回归 386/386 通过。
...
- [x] 本轮 Unity EditMode 全量回归 386/386 通过；Play Mode 进站人工走查仍待验证。
