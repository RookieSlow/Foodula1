...
- [x] 从冻结标签使用 Unity 2022.3.62f3c1 生成 Windows 64 位构建并完成 Direct3D 11 启动冒烟；仓库已公开，GitHub Release 已发布 73.20 MiB ZIP，公开下载校验返回 HTTP 200。
...
- [x] Unity 产品名保持 `Foodula1`，公司名由默认值改为 `Foodula1 Team`，包版本与主菜单版本统一为 `0.1.0` / `v0.1.0 Demo`。
- [x] 将动态中文字体图集中近期 UI 所需的 11 个新增汉字纳入封版，避免雷霆大混战、听验等界面出现缺字。
- [x] 封版前全量 Unity EditMode `676/676` 通过，0 失败、0 跳过。
- [x] 已生成并核验 Windows 64 位构建与 ZIP 归档：构建 194.64 MiB，ZIP 74.26 MiB，正常 Direct3D 11 启动冒烟 12 秒无错误；发布清单记录 SHA-256 与 Git 标签。
- [ ] V0.1.x：只处理试玩发现的明确缺陷、兼容性、文案、音量和轻量表现问题，不改变核心规则或存档结构。
- [ ] V0.2：车手/科技树正式美术、AudioMixer 与外围音效、剩余被动技能和技能平衡、生涯完整扩展、难度/手柄/性能与工具链改造。

## 2026-09-11 勒芒雷霆日志规则修复
...
- [x] 新增 `inRaceConfirmationMask` v2 设置，覆盖换挡选择/确认、出牌弃牌、车手技能、重置、返回、维修区和印地换道；默认保护七类提交动作，玩家可逐项调整。
- [x] HUD 的卡牌、挡位、技能、重置、返回、维修区和换道入口统一复用可配置确认门；教程翻页、卡牌选中和关闭只读面板不弹确认。
- [x] 确认按钮按动作语义动态命名：换挡为“返回修改 / 锁定档位”，返回主菜单为“留在比赛 / 返回主菜单”，避免相同“返回”文案引起误操作。
- [x] 抽出 `CornerLimitBreakdown`，限速数字复用实际结算公式；单击弯心数字显示原始限速、天气、车手、车队操控、科技树和有效值。
- [x] 勒芒旧慕尚第 60 格加入 `mulsanne_kink` Lv1/限速 6 高速偏弯；当前直道环形分布为 `44/37/21/16`，补充赛道 GDD 与现实校准说明。
- [x] 新增设置迁移、确认掩码、限速分项和慕尚弯心 EditMode 回归测试；本轮文案定向 `5/5`、全量 `674/674` 真实通过，0 失败、0 跳过。
- [ ] 在 16:9 Play Mode 验收：逐项确认弹窗层级/设置持久化，点击多个弯心数字核对显示与当前车手/天气/科技状态一致。

## 2026-09-09 移动与尾流演出跳过

- [x] 移动阶段与尾流阶段共用一个跳过状态；主鼠标点击任意位置后，剩余逐格移动、缓冲、超车/尾流特写和卡牌飞行直接收束。
- [x] 跳过不改变基础移动、尾流加成、弯道/圈数/地标/维修区结算，也不替代车道、维修区和教程等必需决策。
- [x] 新增 RacePresentationSkipTests，覆盖窗口生命周期、车辆直接定位和尾流特写清理；Unity EditMode 已实跑并通过（3/3）。
- [ ] 下一步：在 12 车雷霆大混战 16:9 Play Mode 中确认任意位置点击可跳过、最终排名/日志完整，且无时间倍率残留。

## 2026-09-09 比赛返回主菜单二次确认

- [x] 比赛 HUD 与完赛面板的返回主菜单入口统一改为先显示全屏确认弹窗；取消不改变比赛状态，确认后才加载主菜单。
- [x] 旧版/手动编辑 RaceCanvas 缺少弹窗时由 `HUDUI` 运行时补齐，并用遮罩阻断底层误触。
- [x] 新增 `ReturnToMenuConfirmationTests`，覆盖首次点击不切场景、取消后可重开和完赛入口共用确认门；Unity EditMode 已实跑并通过（3/3）。
- [ ] 下一步：在 16:9 Play Mode 走查确认弹窗层级、按钮可读性与游戏结束面板共存表现。

## 2026-09-09 自由赛事自定义混战阵容

- [x] 新增自由赛事开赛前阵容面板：可选择 2–6 支不重复车队，并为每支车队安排
  一名属于本队且不重复的车手；首支队伍为玩家，其余由现有 AI 控制。
- [x] 新增“雷霆大混战”快捷模式：六支车队、十二名车手全部出场，当前车库车手为玩家，
  其余 11 名车手由 AI 控制；12 车开场使用交错视觉车位。
- [x] 保留旧版默认体验（车库车手所在队伍 + UK/DE/IT 四车），确认阵容后继续进入
  原有赛道选择；教程与生涯启动优先级不受影响。
- [x] 将验证后的阵容接入 MVPGameManager，支持 2–6 辆车创建、车手名称/车队徽标、
  科技与车手规则沿用现有 RaceSession；开局日志新增 FREE_RACE_SETUP。
- [x] 新增 FreeRaceRosterRulesTests，覆盖队伍数量、重复队伍、车手归属、选择顺序、
  默认阵容与六车边界。
- [x] 定向阵容与菜单流转测试 `11/11` 通过；全量 Unity EditMode `663/663` 通过，0 失败、
  0 跳过；Play Mode 已启动并安全停止，但编辑器停留在过渡态，视觉验收仍待人工完成。
- [x] 新增雷霆大混战规则回归：目录完整性、六队各两车、玩家首位、模式切换覆盖；定向测试
  `9/9`、全量 EditMode `663/663` 通过，0 失败、0 跳过。
- [ ] 待下一轮：完成 12 车 Play Mode 与分辨率走查，再用雷霆大混战日志形成车队/车手强度报告，
  决定是否需要 AI 难度或数值调整。

## 2026-09-08 车手主动/被动技能与效果重审

- [x] 已对照 `design/gdd/foodula-1-drivers.md` 审计 12 位车手：本轮收口全部主动技能，并重做/接入 5 个低价值被动（曼塞尔、舒马赫、维特尔、周、马）；其余 7 个被动保留为后续工作包。
- [x] 为已解锁主动技能增加比赛内按钮、名称、剩余次数、持续回合及不可用原因；输入仅在档位确认前开放，教程强制禁用。
- [x] 重做三个缺少实际作用目标的主动技能：Mansell 改为移动/超车奖励，Vettel 改为持续免除跨两挡热量，Zhou 改为免疫天气及对手负面修正。
- [x] 主动/被动技能纯规则、运行时、Prefab 引用与天气/弯道交互定向 EditMode `21/21`、全量 `663/663` 通过，0 失败、0 跳过；仍需一次带技能的安全 Play Mode 走查并记录按钮、Console 和 `[DRIVER_SKILL]` 日志证据。
- [x] 已同步车手 GDD、百科、系统索引、路线图与项目记忆；明确区分已接入的 5 个被动和仍为配置描述的 7 个被动，教程继续禁用全部车手技能。
- [x] 新增 `Tools/Run Driver Skill Balance Benchmark`，按 9 条可用赛道、12 位车手、等级 3/7，使用同种子/同车队顺序执行开启与关闭成对模拟；本轮生成 864 对（1728 次）比赛结果，报告见 `design/balance/driver-skill-benchmark-2026-09-08.md`。
- [x] 新增基准确定性回归；全量 Unity EditMode `663/663` 通过，0 失败、0 跳过；脚本校验 0 错误、0 警告。基准只使用当前实际接入的 5 个被动，其余 7 个不虚构运行时收益。
- [ ] 下一工作包：先对照基准报告做一次人工/受控 Play Mode 验收，重点复核哈特、舒马赫、阿斯卡里、诺瓦、拓海的“低收益/负收益”是否来自 AI 发动时机；确认后再决定是否调主动条件或补齐剩余 7 个被动，避免仅按单一模拟结果改数值。

## 2026-09-03 GDC 2026 核心音频包与运行时接入

- [x] 从用户提供的 Sonniss GDC 2026 Part 9 中筛选并非破坏性派生 24 个短音效；统一为
  48 kHz、立体声、16-bit PCM WAV，峰值不高于 -3 dBFS，并保留可重复构建脚本、来源映射与许可证记录。
- [x] 接入用户生成的 `menu.mp3`、`race.mp3`；新增持久化单实例 `AudioService`，按场景淡变音乐，
  使用独立 Music/SFX/UI AudioSource，并让现有主音量、音乐、音效设置立即生效。
- [x] 接入按钮确认、卡牌选择/打出/弃置/抽取、换挡/失败、支付/冷却热量、逐格移动、弯道、
  尾流独立阶段、失控、过圈和完赛事件；高频事件使用未缩放时间限频，尾流慢放不改变音高。
- [x] 音频规则 EditMode `9/9`、全量 EditMode `603/603` 通过，0 失败、0 跳过；MSBuild 0 错误，
  仅保留既有 Unity/MCP 程序集版本警告。24 个 WAV 通过格式/峰值自动校验。
- [ ] 人工完成一局听验，确认 BGM 循环点、遮蔽、事件语义和密度；随后补 AudioMixer、独立 UI/静音，
  以及 UI 悬停/返回、洗牌、热量警告、最后一圈、维修区、科技树与天气环境事件。

## 2026-09-02 纽博格林 GP 终段线路修复

- [x] 修正真实布局导入器：纽博格林 GP 的 SVG 闭合路径后存在孤立标注点，只读取首个闭合子路径，消除终段重叠折返与车辆抽搐。
- [x] 保持 59 格、赛道里程、弯道/弯心与限速元数据不变，并新增终段最小格距、最大方向突变及全圈零自相交回归。
- [x] 使用同一份修正后赛道 JSON 重新生成德国赛道美术图与布局指南；将无道路环境底图纳入项目，避免生成脚本依赖外部临时文件。

## 2026-09-01 品牌美术接入

- [x] 主菜单背景与透明 Foodula1 Logo 通过统一 `BrandArtResources` 接入；缺图时继续显示旧 TMP 标题。
- [x] 六队抽象徽章接入车手选择卡、生涯选队按钮、科技树车队标签和比赛赛车头顶名次徽标，
  并保留原颜色/队伍代码回退。
- [x] 品牌资源映射与实际加载回归 `14/14`、全量 EditMode `593/593` 通过；受控 Play Mode 确认
  主菜单背景/Logo 已创建并加载，车手卡 `12/12`、生涯选队 `6/6`、科技树标签 `6/6` 徽章可用，Console 0 错误/警告。
- [ ] 后续：补齐 12 位车手头像、科技树背景/节点状态资源，并完成 1920×1080 与 2560×1440 截图验收。
...
- [x] 教程体验优化：16 步采用人工修订后的渐进、鼓励式短句；“轮到你了”保持核心操作，
  “现在场上 / 完成后 / 需要帮忙？”改为可选段落，空字段不显示标签、空行或顶部泛化反馈。
- [x] 为每步增加语义化高光目标和独立机制短说明；13 类目标覆盖状态、挡位、手牌、牌区、引擎、
  赛道、天气、维修选择及两张 UK 卡。四块无遮拦遮罩形成聚光窗口，减少动态时边框静止。
- [x] 修正高光单击关闭后的视觉残留：关闭时先将 CanvasGroup 归零，清空说明文本与所有
  TMP/CanvasRenderer 网格并禁用 TMP 组件，再停用四块遮罩、四条边框及说明气泡并刷新 Canvas；
  说明气泡按实际文本高度扩展并用 RectMask2D 裁切越界内容；同一步骤刷新也会再次强制清屏，
  不再等待下一教程步骤。下一教程步骤显示时再完整恢复，
  高光仍不拦截比赛输入。教程 EditMode 回归 `42/42` 通过。
...
- [x] 指引框按当前 TMP 正文首选高度自动增长，底部进度与按钮同步下移且不超过屏幕安全高度；
  当时每步高光可由独立单击关闭且同步刷新不会复现；现版本已进一步细分为“说明气泡关闭、操作边框保留”。
  该阶段全量 EditMode `519/519` 通过，0 失败、0 跳过。
...
- [x] 重排检查点课程表现顺序：带脚本位置的步骤先隐藏指引、落位车辆并重建手牌，再在输入状态
  就绪后显示；冷却/缺牌/弯道/维修选择/司康/红茶自动确认脚本挡位并从出牌阶段开始，确保可见
  手牌中真实包含目标热量或特技牌。保留第一回合完整弃牌演示，其余引导结算后跳过可选弃牌。
  教程定向 EditMode `45/45` 通过，0 失败、0 跳过；完整 Play Mode 人工流程仍待验收。
- [x] 为第 7 步冷却课增加最早显示回合门：第 6 步换挡支付热量后不再复用同一回合的出牌阶段，
  必须完成该回合并在下一次 `TURN_START` 后才准备 G1、热量手牌和指引；新增时序用例 `1/1`、
  教程定向 `45/45`、全量 EditMode `579/579` 通过，0 失败、0 跳过。
- [x] 16 步教程、聚光短说明及练习/完成反馈统一改为耐心的车队教练口吻：先解释判断理由，再给
  新车手一个明确动作，结算后用简短反馈确认；缺牌和打转明确为安全训练演示。同步更新代码回退
  文案与 `TutorialOverlay.prefab`，保留人工 RectTransform、高光和按钮布局；教程定向 EditMode
  `45/45`、全量 `579/579` 通过，0 失败、0 跳过。
- [x] 教程步骤改为右上角面板手动翻页：真实操作只锁存本步完成，玩家点击“下一步”后才进入下一课；
  “上一步”只回看已到达说明，不回滚比赛或重放检查点，并保留跳过/退出。操作聚光覆盖换挡、
  选牌/确认、弃牌/确认、维修和换道；单击后清除说明文字但保留边框。主菜单覆盖层改为互斥，
  生涯入口、确认状态及夏休科技面板不再叠到车手选择界面。教程与菜单定向 EditMode `64/64`、
  全量 EditMode `622/622` 通过，0 失败、0 跳过；Play Mode 仍停在切换态，未计为视觉通过。
