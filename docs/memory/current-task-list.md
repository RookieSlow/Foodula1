...
- [x] 主动/被动技能纯规则、运行时、Prefab 引用与天气/弯道交互定向 EditMode `21/21`、全量 `647/647` 通过，0 失败、0 跳过；仍需一次带技能的安全 Play Mode 走查并记录按钮、Console 和 `[DRIVER_SKILL]` 日志证据。
- [x] 已同步车手 GDD、百科、系统索引、路线图与项目记忆；明确区分已接入的 5 个被动和仍为配置描述的 7 个被动，教程继续禁用全部车手技能。
- [ ] 下一工作包：补齐剩余 7 个被动技能，或先进行 12 个主动 + 5 个被动的安全 Play Mode 平衡验收。
