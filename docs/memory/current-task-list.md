...
- [x] 尾流初始判定改用所有车辆完整的“非尾流计划移动”，科技、特技和车队加成不再
  被遗漏；所有计划终点先统一冻结，再结算尾流，避免受到 `turnOrder` 计算顺序影响。
...
- [x] 将 `race_simulation_test` 和 `TrackTeamBalanceBenchmark` 从“边选牌边移动”改为
  与运行时一致的四阶段：全员选牌 → 冻结非尾流计划 → 统一链式尾流 → 依排名执行。
  纯模拟同时补上进站成功后的出口位置应用；确定性完整比赛测试 1/1 通过。
...
- [x] 增加卡牌区域流转初版：速度/特技/主动弃牌飞向弃牌堆，热量支付从引擎飞向规则
