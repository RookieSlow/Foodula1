...
### Phase 1 — 框架搭建（本次）
- [ ] 建立 `Assets/` 目录结构
- [ ] 创建 RaceCanvas Prefab（UI 层级）
- [ ] 重命名 + 移动现有脚本到新目录
- [ ] 拆分 GameConfigSO → TrackDataSO + CarConfigSO
- [ ] GameManager 分割：RaceManager + InputManager
...
> 📄 关联文档: `foodula-1-visual-style.md`（视觉规范）、`foodula-1-core-mechanics.md`（规则）、`foodula-1-teams-cars.md`（车队数据）
