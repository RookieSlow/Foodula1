...
> **创建日期**：2026-07-13  
> **最后校准**：2026-08-21。当前 AI 运行时以 `AIPlanner`、`AIController`、`RaceSession`
> 和 `ChinaGearShiftRules` 为准；下方旧版行为树描述只在与代码一致的部分作为设计说明。
...
| 车队 | Demo AI 行为差异 |
|---|---|
| 🇬🇧 英国 | 使用已接入的车队科技修正；车手签名技能尚未接入 |
| 🇩🇪 德国 | 使用直道低值牌转换、耐久和科技修正；基础规划器仍负责热量/弯道风险 |
| 🇮🇹 意大利 | 使用操控 +2 与过弯后移动修正；基础规划器负责弯道风险 |
| 🇺🇸 美国 | 使用操控 -1、直道每回合 +1 和额外弯道热量；基础规划器负责风险控制 |
| 🇨🇳 中国 | 使用专属 Go/Recover AI 逻辑（见第七节），不使用自动电池逐圈衰减 |
| 🇯🇵 日本 | 使用车队特技牌与科技修正；更细的秘方策略仍待调优 |
...
```
// 当前 Demo 不启用自动电池逐圈衰减。
// 进站决策由管理器按引擎热量执行：经过维修区入口且热量比例 ≥ 60% 时进站。
IF 经过维修区入口 AND 热量比例 >= 60%：
    → 选择进站（清空热量，从 pit_entry 模拟前进 5 格，跳过 1 回合）
ELSE：
    → 跳过进站，继续比赛

IF 热量 ≥ 60%：
    → 优先选 Recover 档（冷却链由 ChinaGearShiftRules 结算）
    如果连续 Recover 冷却收益下降，切回 Go
...
| 功能 | 阶段 | 说明 |
|---|---|---|
| 科技树 AI build 选择 | Follow-up | 当前 AI 使用配置/演示科技状态；赛前自动构筑仍待加入 |
| 车手被动/签名技能 | Follow-up | 车手目录已接入，技能效果尚未接入 AI 或玩家结算 |
