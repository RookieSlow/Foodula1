...
- ✅ Demo 框架：主菜单 → 比赛 → 返回
- ✅ 核心 HEAT 循环：选档 → 选牌 → 基础移动 → 冷却/弯道 → 回合末尾流 → 弃牌收尾
...
- ✅ AI 对手：热量感知选牌 + 弯道风险判断 + 低热无风险时的主动尾流接近规划
...
| 8 | ~~集成测试：完整比赛流程~~ | 测试 | ✅ 已完成纯层版本（`race_simulation_test.cs`，3 玩家全比赛模拟）；选牌、非尾流计划、链式尾流和依排名移动采用与运行时一致的四阶段边界，进站出口位置纳入模拟；完整 Play Mode 走查仍待完成 |
| 9 | ~~统一硬编码赛道为 JSON~~ | 重构 | ✅ 已完成（`fallback_42.json` 导出 42 节点赛道，无配置时自动加载；代码内建保留作双保险） |
| 10 | ~~运行全部 EditMode 测试~~ | 验证 | ✅ Unity 编辑器回归：441/441 通过；Play Mode 完整走查仍单独跟踪 |
...
| 15 | ~~尾流系统~~ | 功能 | ✅ 已完成（完整非尾流移动统一投影；最多两段链式尾流且不重复前车；帕尔玛/冰糕/筋斗云/范围科技全接入；同回合全部链段合并为 0.9 秒蓝色气流独立演出） |
