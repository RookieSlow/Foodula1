...
| 勒芒旧慕尚 | 142 | 44 / 37 / 21 / 16（当前 JSON） | 旧慕尚直道仍占据压倒性长度；第 60 格加入慕尚高速偏弯 |

> **2026-09-09 追踪说明**：此前的 `61 / 37 / 21` 是加入高速偏弯前的审计抽象。
> 当前权威配置在第 60 格加入 `mulsanne_kink` 单格弯心，因此原 61 格直道被拆为
> `44 / 37 / 21 / 16`。这是有意的视觉与现实节奏修正，不应回退该弯心；最长 44 格仍满足
> 大冲刺容量门槛。
